...
/// Guarantees that the secret key produces an even x-only public key when the spent output is
/// taproot, by negating the secret key if necessary.
#[derive(Debug, Clone)]
...
impl NonEmptyArray<'_, NormalizedSecretKey> {
