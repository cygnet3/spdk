...
impl<'a> NonEmptyArray<'a, NormalizedSecretKey> {
    pub fn sum_keys(&self) -> Result<SecretKey> {
        ...
        let result = tail.iter().try_fold(*head.as_inner(), |acc, item| {
...
impl PartialSenderEcdhShare {
    pub fn new<C: Signing + Verification>(
        ...
        // We generate the dleq proof right away, since we already have the private key
        let proof = generate_dleq_proof(
            secp,
            private_key.as_inner(),
            &recipient_scan_key,
            aux_rand,
            None, // We don't allow a message for now
        )?;
        Ok(Self {
            recipient_scan_key,
            input_vin,
    ...
    pub fn verify_dleq_proof<C: Signing + Verification>(
        &self,
        secp: &Secp256k1<C>,
        input_pubkey: &PublicKey, // Use the vin to find out the input pubkey
    ) -> Result<()> {
...
impl GlobalSenderEcdhShare {
    ...
    pub fn verify_dleq_proof<C: Signing + Verification>(
        &self,
        secp: &Secp256k1<C>,
        input_pubkeys: NonEmptyArray<&PublicKey>, // All eligible input public keys
    ) -> Result<()> {
