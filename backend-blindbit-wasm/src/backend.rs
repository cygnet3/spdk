...
/// Blindbit backend implementation, generic over the HTTP client.
///
/// This allows consumers to choose their HTTP implementation:
/// - Use `ReqwestClient` for the default full-featured option
/// - Implement `HttpClient` for custom/lightweight alternatives
pub struct BlindbitBackend<H: HttpClient> {
    client: BlindbitClient<H>,
}

impl<H: HttpClient> BlindbitBackend<H> {
    /// Create a new Blindbit backend with a custom HTTP client.
    ///
    /// # Arguments
