...
#[cfg(feature = "async")]
use crate::backend::{AsyncChainBackend, BlockDataStream};

#[cfg(not(feature = "async"))]
use crate::backend::{BlockDataIterator, ChainBackend};

/// Unified scanner for silent payment blocks
///
/// This struct supports both sync and async backends depending on feature flags:
/// - With `async` feature: uses `AsyncChainBackend` and async methods
/// - With `sync` feature only: uses `ChainBackend` and sync methods
pub struct SpScanner<'a> {
    updater: Box<dyn Updater + Sync + Send>,
    #[cfg(feature = "async")]
    backend: Box<dyn AsyncChainBackend>,
    #[cfg(not(feature = "async"))]
...
impl<'a> SpScanner<'a> {
    ...
    ///
    /// The backend type depends on feature flags:
    /// - With `async` feature: accepts `Box<dyn AsyncChainBackend>`
    /// - With `sync` feature only: accepts `Box<dyn ChainBackend + Sync + Send>`
    #[cfg(feature = "async")]
    pub fn new(
        client: SpClient,
        updater: Box<dyn Updater + Sync + Send>,
    ...
    // Async process_blocks
    #[cfg(feature = "async")]
    async fn process_blocks(
        &mut self,
        start: Height,
        end: Height,
        mut block_data_stream: BlockDataStream,
    ) -> Result<()> {
        ...
        while let Some(blockdata) = block_data_stream.next().await {
            ...
            self.updater.record_scan_progress(start, blkheight, end).await?;
    ...
    fn process_blocks(
        ...
        block_data_iter: BlockDataIterator,
    ) -> Result<()> {
        let mut update_time: Instant = Instant::now();
