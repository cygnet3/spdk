...
fn validate_psbt_version(psbt: &Psbt) -> Result<()> {
...
fn validate_psbt_state(psbt: &Psbt) -> Result<()> {
...
fn validate_input_fields(psbt: &Psbt) -> Result<()> {
...
fn validate_output_fields(psbt: &Psbt) -> Result<()> {
    for (i, output) in psbt.outputs.iter().enumerate() {
...
fn validate_sp_spend_fields(psbt: &Psbt) -> Result<()> {
...
fn validate_segwit_versions(psbt: &Psbt) -> Result<()> {
    for (i, input) in psbt.inputs.iter().enumerate() {
...
fn validate_sighash_types(psbt: &Psbt) -> Result<()> {
    for (i, input) in psbt.inputs.iter().enumerate() {
...
fn validate_ecdh_coverage(psbt: &Psbt) -> Result<()> {
...
fn validate_dleq_proofs(secp: &Secp256k1<secp256k1::All>, psbt: &Psbt) -> Result<()> {
...
pub fn validate_ready_for_extraction(psbt: &Psbt) -> Result<()> {
    for input_idx in 0..psbt.num_inputs() {
        let input = &psbt.inputs[input_idx];
...
mod tests {
    ...
    use crate::roles::constructor::{add_outputs, add_silent_payment_inputs};
    ...
    fn pubkey_to_p2wpkh_script(pubkey: &PublicKey) -> bitcoin::ScriptBuf {
        let pubkey_hash = bitcoin::PublicKey::new(*pubkey)
            .wpubkey_hash()
            .expect("compressed pubkey required");
    ...
    fn test_validate_ecdh_coverage() {
        use crate::core::{Bip375PsbtExt, EcdhShareData};
        ...
        let sp_addr = SilentPaymentKeyMaterial::new(
            scan_pub,
            spend_pub,
            SpNetwork::Regtest,
