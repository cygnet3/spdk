...
impl SpClient {
    ...
    pub fn create_transaction_from_selection(
        ...
        let expected_outputs = selection.n_sent_outputs() + usize::from(change_index.is_some());
        if total_outputs_amt != selection.sent() + selection.change()
            || recipients.len() != expected_outputs
        {
    ...
    pub fn create_drain_transaction_from_selection(
        ...
        let total_outputs_amt: Amount = recipients.iter().map(|r| r.amount).sum();
        if total_outputs_amt != selection.sent() || recipients.len() != selection.n_sent_outputs() {
            return Err(Error::msg(
                "Amount and/or number of outputs mismatch between recipients and selection",
            ));
        }
