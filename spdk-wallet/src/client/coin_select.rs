...
pub fn select_all_utxos_for_fee_rate(
    ...
    Ok(DrainSelection {
        ...
        fee: Amount::from_sat(u64::try_from(fee_value).expect("fee checked non-negative")),
        actual_fee_rate,
    })
}
...
impl FeeRateCapMetric {
    ...
    fn fee_within_cap(&self, cs: &CoinSelector<'_>) -> Option<i64> {
        ...
        // Ordf32 / overshoot are f32; sat amounts above 2^23 lose precision, which
        // is acceptable for this BnB acceptance bound.
        #[expect(
            clippy::cast_possible_truncation,
            clippy::cast_precision_loss,
            reason = "BnB fee-cap bound is intentionally computed in f32"
        )]
        let max_fee =
...
impl BnbMetric for FeeRateCapMetric {
    fn score(&mut self, cs: &CoinSelector<'_>) -> Option<Ordf32> {
        self.fee_within_cap(cs).map(|fee| {
            #[expect(
                clippy::cast_precision_loss,
                reason = "Ordf32 metric score requires f32"
            )]
            Ordf32(fee as f32)
        })
    }
...
fn run_all_strategies(ctx: &SelectionContext<'_>) -> Vec<InputSelection> {
    ...
    if selections.is_empty()
        && let Ok(selection) = try_greedy_selection(ctx, base_selector)
    {
        selections.push(selection);
    }
...
mod tests {
    ...
    fn selection_by_strategy(selections: &[InputSelection], strategy: Strategy) -> &InputSelection {
        selections
            .iter()
            .find(|selection| selection.strategy() == strategy)
            .unwrap_or_else(|| panic!("missing {strategy:?} selection"))
    }
